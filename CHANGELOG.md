...
- `scripts/test_sandbox.py` runs real commands under `--confine fs` and checks the disk rather than minima's report: a cargo build and a git commit that must succeed, and writes escaping the root by redirect, `cd ..`, symlink, Python, `rm`, truncation and a background job that must not. A mock plays the model, so it needs no key and runs unchanged on Linux and macOS. `CONFINE=paths` is the control, under which the escaping writes land. Under `fs` a second session runs `cargo add` and `go mod tidy` against an empty `CARGO_HOME` and `GOPATH`, and reports whether cargo kept its last-use record. It was chosen over an integration test in `tests/` because the cargo case makes it slow and it needs a kernel with Landlock.
